...
begin 
	...
	<p align=center style="font-size:25px; font-family:family:Georgia"> <b> Bond Duration and Convexity</b> <p>
	<p style="padding-bottom:1cm"> </p>
...
# ╔═╡ a1ee91ba-dc57-4f40-b968-c85e6137aa7e
md"""
**Solution**
"""
...
Markdown.parse("
- Calculate the Modified Duration of a Treasury bond with maturity in $(T5) years, coupon rate of $(c5) percent, and face value of \$ $(F5). Suppose the Treasury's yield to maturity is $(y5) percent. What is percentage change in the price of this bond for a 100 basis point change in the bond's yield?
  - To calculate the modified duration, use the approximation formula from class with ``\\Delta y=$deltaY5``% (20 basis points).
")

# ╔═╡ 5b1d2ccc-9f6c-4d34-89de-71aee7e01b17
md"""
**Solution:**
"""

# ╔═╡ a4fabf28-c124-450b-a2c4-5b8167432f9b
Markdown.parse("
...
# ╔═╡ 110d48f9-82d4-4532-b95a-51227758bd53
Markdown.parse("
...
# ╔═╡ ba6cbca7-2c4f-44e1-bf6d-685e82606784
Markdown.parse("
...
# ╔═╡ c6f01b24-ff34-4208-a432-808964565cfa
md"""
## Exercise 3
"""

# ╔═╡ fcbed142-4155-41b5-9971-b2d027a62f83
md"""

- A portfolio manager is considering the purchase of a bond with a 5.5% coupon rate that pays interest annually and matures in three years. If the yield of the bond is 5%, the price of the bond per \$100 of par value is closest to (A) \$98.65, (B) \$101.36, or (C) \$106.43.
"""

# ╔═╡ c045268a-3dfd-4978-9693-fe47dec5abfd
Foldable("Solution",
md"""
...
# ╔═╡ 6ab28893-1b5e-4e4d-b304-de473f8929da
vspace
...
Markdown.parse("
- Suppose you own a portfolio of zero-coupon bonds. All yields are annually compounded. Calculate the modified duration of the bond portfolio given in the table below.
...
\$\\begin{aligned}
&B(y) = \\frac{3}{1.005} + \\frac{103}{1.006^2} = 104.76\\\\
&B(y + \\Delta y) = \\frac{3}{1.0055} + \\frac{103}{1.0065^2} = 104.6575\\\\
&B(y - \\Delta y) = \\frac{3}{1.0045} + \\frac{103}{1.0055^2} = 104.8528\\\\
...
\$\\begin{aligned}
&B(y) = \\frac{5}{1.005} + \\frac{5}{1.006^2} + \\frac{5}{1.007^3} + \\frac{105}{1.009^4} = 116.12\\\\
&B(y+\\Delta y) = \\frac{5}{1.0055} + \\frac{5}{1.0065^2} + \\frac{5}{1.0075^3} + \\frac{105}{1.0095^4} = 115.9004\\\\
&B(y-\\Delta y) = \\frac{5}{1.0045} + \\frac{5}{1.0055^2} + \\frac{5}{1.0065^3} + \\frac{105}{1.0085^4} = 116.3314\\\\
...
- Calculate the convexity and the modified duration of this bond the convexity and modified duration approximation formulas with ``\\Delta y=$deltaY2``%.
...
md"""
- Suppose you are analyzing a bond with 10 years to maturity, \$1000 face value, and a coupon rate of 5% (paid semi-annually).
...
4. If discount rates increase to 10%, what is the new price of the bond? 
   - Do (i) the actual calculation and (ii) approximate the new bond price using the duration and convexity. How well does the duration and convexity approximation work?
"""
...
Markdown.parse("
\$\\begin{aligned}
&25\\times\\frac{1}{.0305}\\left[1 - \\frac{1}{1.0305^{20}}\\right] + \\frac{1000}{1.0305^{20}} = 918.5509\\\\
&25\\times\\frac{1}{.0295}\\left[1 - \\frac{1}{1.0295^{20}}\\right] + \\frac{1000}{1.0295^{20}} = 932.7408
\\end{aligned}\$
...
- First, calculate the actual price change
\$\\begin{aligned}
...
\$\\begin{aligned}
&\\frac{\\Delta B}{B} \\approx -7.67(0.04) + \\frac{1}{2}(71.79)(0.04)^2 = -0.249178\\\\
